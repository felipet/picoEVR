...
# Enable the ECATM FMC in the design
ENABLE_ECATM ?= $(shell grep -e 'firmware.en_ecatm' $(CONFIG_NAME) | sed 's/[^:]*:\s*//')
...
$(OUTPUT_DIR)/$(PROJECT_NAME).xpr:
	...
	@mkdir -p $(LOG_DIR)
	@vivado -mode batch -m64 -source $(PROJECT_TCL) -tclarg --origin_dir "fpga/srcs/tcl/" --carrier_rev "$(CARRIER_REV)" --enable_dio "$(ENABLE_DIO)" --enable_debug "$(ENABLE_DEBUG)" --enable_ecats "$(ENABLE_ECATS)" --enable_ecatm "$(ENABLE_ECATM)" 2>&1 | tee $(LOG_DIR)/project.log
